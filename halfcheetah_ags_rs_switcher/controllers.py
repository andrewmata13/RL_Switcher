"""
HalfCheetah policy wrappers.

Mirrors hopper_ags_rs_switcher/controllers.py exactly, with the following
environment-specific differences:
  - obs_dim=17, action_dim=6
  - name="Cheetah" in all custom_env calls
  - uState for reset: qpos(9) + qvel(9) = 18D; obs = uState[1:] = 17D
  - _raw_obs_from_sim: qpos[1:9] + qvel[:9] (no velocity clipping)
  - Termination: ang > -0.8 (handled inside custom_env.step)
  - eps=0.15 (Zhang et al. L-inf attack budget for HalfCheetah)
"""
import sys
import os
import numpy as np
...
def _raw_obs_from_sim(custom_env) -> np.ndarray:
    """
    Raw 17D HalfCheetah observation: qpos[1:9] + qvel[:9].
    HalfCheetah qpos has 9 elements (qpos[0] is the x forward position,
    excluded from the obs). qvel has 9 elements. No velocity clipping.
    """
    inner = custom_env.env.unwrapped
    qpos  = np.array(inner.sim.data.qpos.flat[1:9])   # 8D
    qvel  = np.array(inner.sim.data.qvel.flat[:9])     # 9D
    return np.concatenate([qpos, qvel]).astype(np.float32)


# ── Policy wrappers ───────────────────────────────────────────────────────────

class CheetahPerfPolicy:
    """
    Native PPO policy for HalfCheetah. Loaded from the attack checkpoint so
    ...
    def predict(self, obs_norm: np.ndarray) -> np.ndarray:
        """obs_norm: 17D normalized obs. Returns action in [-1,1]^6."""
        with torch.no_grad():
    ...
    def start_episode(self, max_steps: int = 1000) -> np.ndarray:
        ...
        obs_norm = self.custom_env.reset(uState, None, name="Cheetah")
        return obs_norm.astype(np.float32)

    def step(self, action: np.ndarray):
        result, norm_rew, is_done, info = self.custom_env.step(
            action, change_filter=False, name="Cheetah"
        )
...
class CheetahBackupPolicy:
    """
    ATLA-defended backup policy for HalfCheetah. Uses its own ZFilter.
    """
