...
import gym
from other_attacks.optimal_attack.auto_LiRPA import BoundedModule
from other_attacks.optimal_attack.auto_LiRPA.eps_scheduler import LinearScheduler
from other_attacks.optimal_attack.auto_LiRPA.bounded_tensor import BoundedTensor
from other_attacks.optimal_attack.auto_LiRPA.perturbations import PerturbationLpNorm
from .models import *
...
class Trainer():
    ...
    def __init__(self, policy_net_class, value_net_class, params,
                 store, environment, start_state, attack_steps, advanced_logging=True, log_every=5):
        '''
        ...
        self.envs = self.environment.env
        ...
        self.params.AGENT_TYPE = "discrete" if not self.environment.continuous else "continuous"

        self.params.NUM_ACTIONS = 1 #hack-change if want multi-dim action space
        
        self.params.NUM_FEATURES = len(self.environment.bounds)
        ...
        #Manual Change - Andrew
        self.policy_model = self.envs.predict
    ...
    def multi_actor_step(self, actions, envs):
        ...
        gym_action = gym_action[0]
        
        new_state, normed_reward, is_done, _ = envs.step(gym_action)

        self.length += 1
        self.total_reward += normed_reward
        
        if is_done:
            completed_episode_info.append((self.length, self.total_reward))
            new_state = envs.reset(self.start_state)
            self.length = 0
            self.total_reward = 0
    ...
    def run_trajectories(self, num_saps, return_rewards=False, should_tqdm=False,
        ...
        envs = self.envs
        initial_states = envs.reset(self.start_state)
        ...
        states_shape = (self.NUM_ACTORS, traj_length+1) + initial_states.shape
        ...
        if collect_perturbed_state:
            ...
            states[:, 0, :] = torch.tensor(initial_states)
...
#                    next_adv_perturbation_log_probs = self.adversary_policy_model.get_loglikelihood(adv_perturbation_pds, next_adv_perturbations)
            ...
            next_actions = self.policy_model(last_states)
        ...
        trajs = Trajectories(rewards=rewards, 
                             action_log_probs=action_log_probs, not_dones=not_dones, 
                             actions=actions, states=states, action_means=action_means, action_std=None)
    ...
    def apply_attack(self, last_states):
        ...
        if self.params.ATTACK_METHOD == "critic":
            ...
            self.attack_policy_network = self.adversary_policy_model
    ...
    def train_step_impl(self, adversary_step=False, increment_scheduler=True):
        ...
        saps, avg_ep_reward, avg_ep_length = self.collect_saps(num_saps, collect_adversary_trajectory=adversary_step, should_log=False)
        policy_loss, surr_loss, entropy_bonus, val_loss = self.take_steps(saps, adversary_step=adversary_step, increment_scheduler=increment_scheduler, logging=False)
        # Logging code
    ...
    def run_test(self, max_len=200, compute_bounds=False, use_full_backward=False, original_stdev=None):
        print("-" * 80)
        start_time = time.time()
        ...
        with torch.no_grad():
            ...
            #find best n-step attack
            for attacked in range(max_len - self.attack_steps):
                output, errors = self.run_test_trajectories(attacked, max_len=max_len)
                ep_length, ep_reward, actions, action_means, states = output

                #if ep_length < 200:
                #    print(ep_length)
                
                if bestOutput is None or bestOutput[0] > output[0]:
                    bestOutput = output
                    bestErrors = errors
                    
            #print(bestOutput)
            #print("ERRORS:", bestErrors)
            #exit()

            ep_length, ep_reward, actions, action_means, states = bestOutput
            ...
            # Unroll the trajectories (actors, T, ...) -> (actors*T, ...)
        return ep_length, ep_reward, actions, action_means, states.cpu().numpy(), kl_upper_bound

    def run_test_trajectories(self, attacked, max_len, should_tqdm=False):
        # Arrays to be updated with historic info
        envs = self.envs
        initial_states = envs.reset(self.start_state)
        ...
        states_shape = (1, max_len+1) + initial_states.shape
        states =  ch.zeros(states_shape)
        ...
        for t in iterator:
            ...
            if t >= attacked and t < attacked + self.attack_steps:
                maybe_attacked_last_states = self.apply_attack(last_states)
            else:
                maybe_attacked_last_states = last_states
                
            noisy = maybe_attacked_last_states[0].numpy()
            #print("Noisy State ", noisy)
            clean = last_states[0].numpy()
            ...
            #print("Percent Error: ", max(errs)*100)
            ...
            actions = self.policy_model(maybe_attacked_last_states)
            if hasattr(self, "imit_network"):
            ...
            ret = self.multi_actor_step(actions, envs)
        ...
        to_ret = (ep_length, ep_reward, actionList, action_means, states)
        
        return to_ret, errors
    ...
    @staticmethod
    def agent_from_params(params, environment, start_state, attack_steps, store=None):
        '''
        ...
        p = Trainer(agent_policy, agent_value, params, store, environment, start_state, attack_steps, log_every=log_every,
