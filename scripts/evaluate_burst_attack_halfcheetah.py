"""
Evaluate HalfCheetah controllers under the Zhang et al. adversarial attack.
...
HalfCheetah specifics vs Hopper:
  obs_dim=17, action_dim=6, eps=0.15 (L-inf)
  L-2 budget = eps * sqrt(obs_dim) = 0.15 * sqrt(17) ≈ 0.618
  delta_budget_l2=0.075 for detection (same as Hopper — keep P(false alarm) low)
...
Usage:
    python3.8 scripts/evaluate_burst_attack_halfcheetah.py \
        ...
        --switcher-path models/halfcheetah_switcher.pt \
...
from halfcheetah_ags_rs_switcher.controllers import CheetahPerfPolicy, CheetahBackupPolicy
from halfcheetah_ags_rs_switcher.evaluation import (
    AlwaysPerfController, AlwaysBackupController,
    AnyTimeSwitcherController,
    evaluate_controller,
)
from halfcheetah_ags_rs_switcher.models import SwitcherMLP
from halfcheetah_ags_rs_switcher.rs import VanillaRSSwitcher
...
def main():
    ...
    perf   = CheetahPerfPolicy.load(args.perf_path, attack_path=args.attack_path,
                                    naive_policy=args.naive_policy)
    if args.attack_eps is not None:
        perf.eps = args.attack_eps
    backup = CheetahBackupPolicy.load(args.backup_path)
    ...
    print("=== HalfCheetah burst-attack evaluation ===")
