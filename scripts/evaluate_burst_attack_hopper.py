"""
Evaluate Hopper controllers under the Zhang et al. adversarial attack.
...
from hopper_ags_rs_switcher.controllers import HopperPerfPolicy, HopperBackupPolicy
from hopper_ags_rs_switcher.evaluation import (
    AlwaysPerfController, AlwaysBackupController,
    AnyTimeSwitcherController,
    evaluate_controller,
)
from hopper_ags_rs_switcher.models import SwitcherMLP
from hopper_ags_rs_switcher.rs import VanillaRSSwitcher


def run(controller, perf, backup, episodes, seed, attacked, burst_k, t_candidate_max=20):
...
def main():
    ...
    parser.add_argument("--t-candidate-max", type=int,   default=300,
    ...
    backup = HopperBackupPolicy.load(args.backup_path)
    ...
    print("=== Hopper burst-attack evaluation ===")
    ...
    for name, ctrl in controllers.items():
        ...
        for attacked, label in [(False, "clean"), (True, "attacked")]:
            m = run(ctrl, perf, backup, args.episodes, args.seed, attacked, args.burst_k,
